...
export default function Sidebar() {
    ...
    <>
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className={cn('text-white whitespace-nowrap transition-opacity duration-200', isExpanded ? 'opacity-100' : 'lg:opacity-0')}>
      ...
      <nav className="flex-1 px-2 space-y-1 overflow-y-auto overflow-x-hidden">
        {navigation.filter(item => canAccess(role, item.href)).map((item) => (
          <Link
            key={item.name}
            href={item.href}
          ...
          >
            ...
            <span className={cn('whitespace-nowrap transition-opacity duration-200', isExpanded ? 'opacity-100' : 'lg:opacity-0')}>{item.name}</span>
            {item.badge && (
              <span className={cn('px-1.5 py-0.5 text-[10px] font-medium bg-amber-500/20 text-amber-400 rounded whitespace-nowrap transition-opacity duration-200', isExpanded ? 'opacity-100' : 'lg:opacity-0')}>
                {item.badge}
              </span>
            )}
          </Link>
        ...
        {/* Мой план — только для владельца */}
        {canAccess(role, '/app/subscription') && <div className="pt-2 px-1">
          <Link
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'hidden lg:block fixed left-0 top-0 h-screen bg-gradient-to-b from-gray-800 to-gray-900 shadow-xl z-40 transition-all duration-300 ease-in-out overflow-hidden',
