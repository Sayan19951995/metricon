...
export class KaspiAPIClient {
   ...
   * Endpoint: POST mc.shop.kaspi.kz/pricefeed/upload/merchant/settings/auto/{merchantId}
   */
  async registerAutoLoadUrl(
    ...
    const url = `${BASE_URL}/pricefeed/upload/merchant/settings/auto/${this.merchantId}`;
    ...
    try {
      ...
      console.log(`[AutoLoad] POST ${url} → ${response.status}`, text.substring(0, 300));
  ...
  /**
   * Проверить URL автозагрузки (Kaspi скачает и валидирует XML).
   * Endpoint: POST mc.shop.kaspi.kz/pricefeed/upload/merchant/settings/auto/{merchantId}/check
   */
  async checkAutoLoadUrl(
    feedUrl: string,
    login?: string,
    password?: string
  ): Promise<{ success: boolean; error?: string }> {
